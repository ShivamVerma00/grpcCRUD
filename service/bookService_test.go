...
func init() {
	...
	pb.RegisterBookManagementServiceServer(server, &BookManagementServiceServer{})
	go func() {
...
func TestLmsImp(t *testing.T) {
	...
	//Create new Client
	client := pb.NewBookManagementServiceClient(conn)
	...
	_, err = client.GetAllBooks(ctx, &pb.ReqGetAllBooks{})
	if err != nil {
	...
	//Test SearchBooks
	_, err = client.SearchBook(ctx, &pb.ReqSearchBook{
		Search: &pb.ReqSearchBook_BookTitle{BookTitle: newBook.BookTitle},
	})
	if err != nil {
	...
	_, err = client.UpdateBook(ctx, &pb.ReqUpdateBook{Book: updateBook})
	if err != nil {
